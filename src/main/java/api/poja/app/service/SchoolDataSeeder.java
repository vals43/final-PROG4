...
public class SchoolDataSeeder implements CommandLineRunner {
  private static final List<String> COURSE_REFS =
      List.of(
          "PROK1", "WEB1", "CQ1", "MATH1", "ANG1", "SYS1",
          "PROK2", "WEB2", "CQ2", "MATH2", "ANG2", "SYS2",
          "PROK3", "WEB3", "MATH3", "CQ3", "PROBL3",
          "TN1", "ELEC1",
          "PROK4", "ELEC2", "TN2", "TN3", "WEB4", "MATH4", "CQ4", "ANG4", "MOB4",
          "INC5", "DIST5", "ROUT5", "ADM5", "CQ5", "WEB5", "ANG5", "MAG5",
          "PFE6");
  ...
  public void run(String... args) {
    ...
    var alice = newUser("STD101", "Randria", "Alice", "alice@hei.edu", Role.STUDENT, ParcoursType.EL);
    var bob = newUser("STD102", "Rakoto", "Bob", "bob@hei.edu", Role.STUDENT, ParcoursType.EL);
    var charly = newUser("STD103", "Andria", "Charly", "charly@hei.edu", Role.STUDENT, ParcoursType.TN);
  ...
  private User newUser(String std, String nom, String prenom, String email, Role role, ParcoursType parcours) {
    return User.builder()
        .std(std)
        .nom(nom)
        .prenom(prenom)
  ...
  private Inscription newInscription(User student, Groupe groupe, int semestre, int annee) {
    return Inscription.builder().student(student).groupe(groupe).semestre(semestre).annee(annee).build();
  }
  ...
  private java.util.Map<String, Cours> createCourses(Parcours el, Parcours tn) {
    ...
    var cours = coursRepository.saveAll(
        List.of(
  ...
  private Cours cours(String ref, String intitule, int credits, int semestre, List<Parcours> parcours) {
  ...
  private Examen examen(Cours cours, int annee, int numero, BigDecimal coefficient) {
    var start = LocalDateTime.of(annee, cours.getSemestre() <= 2 ? 1 : 9, numero == 1 ? 15 : 16, 9, 0);
